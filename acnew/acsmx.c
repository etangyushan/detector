...
acsmSearch (ACSM_STRUCT * acsm, unsigned char *Tx, int n,
	    void (*PrintMatch) (ACSM_PATTERN * pattern, ACSM_PATTERN * mlist,
				int nline, int index))
{
  int state;
  ACSM_PATTERN *mlist;
  ...
  for (state = 0; T < Tend; T++)
	...
	{
	  for (mlist = StateTable[state].MatchList; mlist != NULL; mlist =
	       mlist->next)
	    {
 ...
 */
void
PrintMatch (ACSM_PATTERN * pattern, ACSM_PATTERN * mlist, int nline,
	    int index)
{
