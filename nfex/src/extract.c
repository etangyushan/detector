 ...
 * All rights reserved.  * Based off of tcpxtract by Nicholas Harbour.
 */
...
uint16_t dst_prt, char **fname, ncc_t *ncc)
{
	...
	/* 指定抓取文件存放路径 */
	char *buf = BETRANSFORM;
	//printf ("nfex-buf=%s\n",buf);

	if (ncc->output_dir)
	{
		snprintf(*fname, FILENAME_BUFFER_SIZE, "%s/%d-%06d.%s", buf, getpid(), ncc->filenum, ext);
	}
	else
	{
		snprintf(*fname, FILENAME_BUFFER_SIZE, "%s/%d-%06d.%s", "", getpid(), ncc->filenum, ext);
	}
#if 0
    snprintf(*fname, FILENAME_BUFFER_SIZE, "%s/%d-%06d.%s", 
        //ncc->output_dir == NULL ? "" : ncc->output_dir, 
        //ncc->output_dir == NULL ? "" : buf, 
	...
	sprintf (msg.nameinfo.srcip, "%d.%d.%d.%d:%d", ip_addr_s[0], ip_addr_s[1], ip_addr_s[2], ip_addr_s[3], ntohs(src_prt));
	sprintf (msg.nameinfo.desip, "%d.%d.%d.%d:%d", ip_addr_d[0], ip_addr_d[1], ip_addr_d[2], ip_addr_d[3], ntohs(dst_prt));
	strcpy (msg.nameinfo.filename, *fname);
	//printf("nfex---------------fname=%s\n", *fname);
	//printf ("nfex--------------filename=%s\n", msg.nameinfo.filename);
	msg.mytype = 1;
