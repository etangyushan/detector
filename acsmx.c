 ...
 */
int
acsmSearch (ACSM_STRUCT * acsm, unsigned char *Tx, int n, void (*PrintMatch) (ACSM_PATTERN * pattern, ACSM_PATTERN * mlist, int nline, int index))
{
	int state;
	ACSM_PATTERN *mlist;
	unsigned char *Tend;
...
acsmFree (ACSM_STRUCT * acsm)
{
	...
	ACSM_PATTERN *pattern;

	pattern = acsm->acsmPatterns;
	while (pattern != NULL) {
		while (pattern->coordinate != NULL) {
			struct Coordinate *tempCoo = pattern->coordinate;
			pattern->coordinate = pattern->coordinate->next;
			AC_FREE (tempCoo);
		}
 ...
 */
void
PrintMatch (ACSM_PATTERN * pattern, ACSM_PATTERN * mlist, int nline, int index)
{
	/* Count the Each Match Pattern */
	ACSM_PATTERN *temp = pattern;
	for (; temp != NULL; temp = temp->next) {

		if (!strcmp ((char *)temp->patrn, (char *)mlist->patrn))	//strcmp succeed return 0,So here use "!" operation
		{

			if (temp->coordinate == NULL) {
				temp->coordinate = (struct Coordinate *)malloc (sizeof (struct Coordinate));
				memset (temp->coordinate, 0, sizeof (struct Coordinate));
				temp->coordinate->line = nline;
				temp->coordinate->colm = index;
			}
			else {
				struct Coordinate *coord = temp->coordinate;
				while (coord->next != NULL) {
					coord = coord->next;
				}
				coord->next = (struct Coordinate *)malloc (sizeof (struct Coordinate));
				memset (coord->next, 0, sizeof (struct Coordinate));
				coord->next->line = nline;
				coord->next->colm = index;
			}
 ...
 */
void
PrintSummary (ACSM_PATTERN * pattern)
{
	ACSM_PATTERN *mlist = pattern;
	printf ("\n ############ Matching result start ############\n");
	for (; mlist != NULL; mlist = mlist->next) {
		if (mlist->nocase)
			printf ("%12s : %5d\n", mlist->patrn, mlist->nmatch);
		else {
			printf ("------------------%12s : %5d-------------------\n", mlist->casepatrn, mlist->nmatch);
			struct Coordinate *coo = mlist->coordinate;
