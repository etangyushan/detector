...
rest_hdd ()
{
	...
	for (i = 0; i < 2; i++) {
		...
		switch (i) {
		case 0:
			rt_da.free_root = (4 * sfs.f_blocks) / (1024 * 1024);
			break;
		case 1:
			rt_da.free_home = (4 * sfs.f_blocks) / (1024 * 1024);
			break;
		default:
			;
			break;
...
divie_num (char *line, int flag)
{
	...
	switch (flag) {
	case 0:
		data.total_size = atof (str);
		break;
	case 1:
		data.total_root = atof (str);
		break;
	case 2:
		data.total_swap = atof (str);
		break;
	case 3:
		data.total_home = atof (str);
		break;
	default:
		break;
	}
...
get_diskstate ()
{
	...
	int save_fd;
	static hadd total;
	char *diskmsg = "diskmsg.tmp";
#if 1
	fflush (stdout);
	setvbuf (stdout, NULL, _IONBF, 0);
	save_fd = dup (STDOUT_FILENO);
	fd = open (diskmsg, (O_RDWR | O_CREAT | O_TRUNC), 0666);
	if (fd < 0) {
		printf ("<%d>\n", __LINE__);
		perror ("open()");
		exit (-1);
