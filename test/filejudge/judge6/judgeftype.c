...
int filetotxt(char *filenameun)
{
	...
	while ((read = getline (&line, &len, pf)) != -1) 
	{
		...
		//printf ("flag=%d, line=%s\n", flag, line);
		/* 处理.doc, .pdf, .ppt文件 */
		//printf("command=%s\n", command);

		/* 文件类型flag= 1:.doc, 2:.pdf, 3:.ppt */
		split_name(line, fname);
		//printf ("flag=%d, fname=%s\n", flag, fname);
		//printf("fname=%s\n", fname);
		...
		switch (flag) 
		{
				case 1:
					...
					sprintf(command, "%s %s > %s/%s.txt", comtype, line, ATPATH, fname);
					//system (catdoc);
					printf("这个是.doc: %s\n", command);
					system(command);
					//flag = 0;
				...
				case 2:
					...
					sprintf(command, "%s %s %s/%s.txt", comtype, name, ATPATH, fname);
					system (command);
					//printf("这个是.pdf\n");
					//flag = 0;
					break;
